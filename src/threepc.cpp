	...
	while (true) {	
		...
		if (FD_ISSET(sockfd, &fdset)){   
			...
			if (type == REPLY && state == WAIT) {
				Reply *msg = (Reply *)p;
			//	cout << "msg val " << msg->val << " value1x " << value << endl;
				...
				Ack *msg = (Ack *)p;
			//	cout << "msg val " << msg->val << " value2x " << value << endl;
				assert(value == msg->val);
				int sender = msg->senderid;
				tag->setTrue(sender);
			...
			//	cout << "msg val " << msg->val << " value3x " << value << endl;
				assert(value == msg->val);
				int sender = msg->senderid;
				tag->setTrue(sender);
			...
			//	cout << "msg val " << msg->val << " value4x " << value << endl;
				assert(value == msg->val);
				int sender = msg->senderid;
				tag->setTrue(sender);
...
void ThreePC::doSlave() {   ////////////////////////////
	...
	while (true) {
		...
		if (FD_ISSET(sockfd, &fdset)){   
			...
			int type = recvMessage(p);
			//printMsg(R, p);
			if (type == CANCMT && (state == CMT || state == ABRT )) {  // new round 
				state = INIT;
				//stgyidx++;   ///////////----------
				Cancmt *msg = (Cancmt *)p;
